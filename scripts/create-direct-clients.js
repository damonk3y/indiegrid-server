...
(async () => {
  const fileContent = readFileSync('./scripts/data/direct_clients.csv', 'utf-8');
  const entries = fileContent.split('\n');
  const records = entries.reduce((acc, entry) => {
    if (entry.includes('#VALOR')) {
      return acc;
    }
    const [_, handle, address, name, street, zipAndCity, __, ___, ____, _____, phone] = entry.split(';');
    const [zip, city] = zipAndCity ? zipAndCity.split(' ') : ['', ''];
    const content = {
        handle: handle ? handle.replace('/n', '').trim() : undefined  ,
        address: address ? address.trim() : undefined,
        name: name ? name.trim() : undefined,
        street: street ? street.trim() : undefined,
        zip: zip ? zip.replace(',', '').trim() : undefined,
  ...
  for (const record of records) {
    if (!record.name || !record.zip || record.name === record.street) continue;
    await prisma.directClient.create({
      data: {
        store_id: "f77b063d-5715-4827-b414-f01ef8daf02c",
        name: record.name,
        phone: record.phone,
        facebook_url: encodeURI(`https://facebook.com/${record.handle}`),
        instagram_url: encodeURI(`https://instagram.com/${record.handle}`),
        addresses: {
            create: {
